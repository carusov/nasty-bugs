...
readVCFs <- function(data_path, sample_name){
  vcf_path <- file.path(data_path, sample_name, "vcf")
  vcf_files <- list.files(file.path(data_path, sample_name, "vcf"), ".vcf.gz$")
...
for (s in samples[6]){
  ...
  new_row <- tibble(s, ref_len, avg_calls, min_calls, max_calls, n_called, pct_cvg, n_snps, n_error, accuracy)
...
write_tsv(summary_stats, file.path(data_path, "summary_stats.tsv"))
